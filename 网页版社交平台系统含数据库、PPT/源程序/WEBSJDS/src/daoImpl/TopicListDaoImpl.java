...
public class TopicListDaoImpl extends BaseDao implements TopicListDao {
	...
	public List<Topic> getMeAndAllFriendTopic() {
		...
		try {
			conn = super.getConn();  
			pst = conn.prepareStatement("select u.user_nicename,t.topic_webtext,t.topic_id,u.user_username,t.time,u.user_id,u.user_img from topic t,user u where t.user_id = u.user_id order by t.topic_id desc");
			rs = pst.executeQuery();
			while(rs.next()){
