...
public class TopicListDaoImpl3 extends BaseDao implements TopicListDao3{
	//获取各自的讨论话题
...
public List<Topic1> getcontext(int topicId){
		...
		try {
			conn = super.getConn();
			pst = conn.prepareStatement("select u.user_nicename,t.topic_webtext,u.user_username,t.topic_id1,u.user_id,t.time,u.user_img from topic1 t,user u where t.user_id = u.user_id and t.topic_id1 = ?");
			pst.setInt(1, topicId);
			rs = pst.executeQuery(); 
			...
			topic.setPraisecount(praiseCount);
			topic.setUsers(users);
			List<Users> praisePeopleNameList = topicListDao.getPraisePeopleName(rs.getInt(4));
			topic.setList(praisePeopleNameList);
			List<Topic1> PeopleNameList = CommentDao1.search(rs.getInt(4));
