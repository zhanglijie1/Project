			...
			<div class="layui-col-md8 content detail">
				...
				<div class="fly-panel detail-box">
					<c:forEach items="${meAndAllFriendTopic}" var="item">
						...
						<h1>标题 ：${item.webtext}</h1>


						<div class="fly-detail-info">
							<div class="fly-admin-box" data-id="123"></div>
							<span class="fly-list-nums"> <!-- <input type="button" onclick="myFunction()" value="检验" /> -->
								<a class="zan" id="input1"
								href="${pageContext.request.contextPath }/servlet/DianpraiseAction?topicId=${item.topicId }">点赞(${item.praisecount })</a>
								<i class="iconfont" title="人气">&#xe60b;</i> ${item.praisecount }<input
								type="hidden" id="topicId" value="${item.topicId }">
							</span>
						</div>
						<div id="p"></div>
						<div class="detail-about">
							<a class="fly-avatar" href="${pageContext.request.contextPath }/servlet/TopicListAction5?userId=${item.users.userId }"> <img
								src="${item.users.userimg}">
							</a>
							<div class="fly-detail-user">
								<a href="user/home.html" class="fly-link"> <a
									href="${pageContext.request.contextPath }/servlet/TopicListAction5?userId=${item.users.userId }"
									link><cite> ${item.users.niceName}: </cite></a> <i
									class="iconfont icon-renzheng"
									title="认证信息：{{ rows.user.approve }}"></i> <i
									class="layui-badge fly-badge-vip">VIP3</i> <span>(博主)</span>
							...
							<div class="detail-hits" id="LAY_jieAdmin" data-id="123">
								...
								<c:if
									test="${loginUser.userId == item.users.userId||loginUser.userName=='123456'}">
									<span class="layui-btn layui-btn-xs jie-admin" type="edit"><a
										href="${pageContext.request.contextPath }/servlet/DeleteAction1?topicId=${item.topicId }">删除此贴</a></span>
								</c:if>

								<a>发布时间 :</a> <span>${item.time}</span>
							</div>
						</div>
						<h3>正文 ：${item.content}</h3>
					</c:forEach>
				</div>
				...
				<div class="fly-panel detail-box" id="flyReply">
					...
					<div class="layui-form layui-form-pane">
						<form action="CommentServlet?method=add" method="post">
							...
							<div class="layui-form-item">
								...
								<button class="layui-btn submit" lay-filter="*" lay-submit>发表评论</button>
							</div>
						</form>
					</div>
							...
							<c:forEach var="f" items="${item.list1}">
								...
								<li data-id="111"><a name="item-1111111111"></a>
									<div class="detail-about detail-about-reply">
										...
										<div class="fly-detail-user">
											<a href="" class="fly-link"> <a
												href="${pageContext.request.contextPath }/servlet/TopicListAction5?userId=${f.comment_userId }"><cite>
									...
									<div class="detail-body jieda-body photos">
										...
										<div class="detail-hits">
											<span style="color: gray">回复时间 ： ${v.time}</span>
										</div>
										<p>${v.nicename}  回复 ${f.nicename}: ${v.ccontent}</p>
										<hr>
									</c:forEach>
									<div class="jieda-reply">
										...
										<div class="jieda-admin">
											...
											<c:if test="${loginUser.userId == f.comment_userId ||f.comment_userId=='123456' }">
												<a id="input"
													href="CommentServlet?method=del&comment_id=${f.comment_Id}&topicId=${item.topicId}">
								...
								<div class="qutton" id="qutton_comment">
									<div class="qutton_dialog" id="commentDialog">
										<form action="CommentServlet?method=add1" method="post">
											...
											<input type="hidden" name="c_commentid"
												...
												type="hidden" name="time" id="mypwd"> <input
												type='hidden' name="topicId" id="topicid"
												value="${item.topicId}"> <input class="layui-btn"
       ...
       //给检验按钮注册事件
           function myFunction(){
                var topicId = $('#topicId').val();
                    ...
                    //后台处理数据验证
                    $.get("DianzanServlet?method=search", { topicId: topicId }, 
                    function (data) {
                    	  if(data.length>0){
      						$.each(data,function(index,das){
